...
if (IS_LOGIN) {
    ...
    if (IS_CONNECT) {
        ...
        if (is_object($query)) {
            if (isset($_GET['action']) && 'selected_table' == trim($_GET['action'])) {
                ...
                if (isset($_POST['delete']) && null != $entrys) {
                    ...
                    foreach ($entrys as $v) {
                        if (false == isTableExists(addslashes($v))) {
                            $isAllExists = false;
                            break;
                        } else {
                    ...
                    while ($assoc = mysqli_fetch_assoc($query)) {
                        ...
                        echo '<li>
                                ...
                                <p>
                                    ...
                                    <span class="count_columns">'.(0 == $assoc['Rows'] ? mysqli_num_rows(mysqli_query($conn, "SHOW COLUMNS FROM `$name`")) : $assoc['Rows']).'</span>
                                    <span>cột</span>
                                </p>
                            </li>';
                    }
